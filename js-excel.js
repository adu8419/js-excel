        ...
        prepareTable = function () {
            ...
            for (i = 0; i < size; i++) {
                ...
                if (! options.columns[i]) {
                    options.columns[i] = { type:'text', editable: true };
                }else {
                       options.columns[i]  = $.extend({}, { editable: true} , options.columns[i] )
                }
    ...
    createTable : function() {
        ...
        if (! $('.jexcel_corner').length) {
            ...
            $(document).on('dblclick touchend', function (e) {
                ...
                // Jexcel is selected
                if ($.fn.jexcel.current) {
	      var columnId = $($.fn.jexcel.selectedCell).prop('id').split('-');
                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].editable == true 
					&& $.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].editable === true) {
                        // Corner action
                        if (e.target.id == 'corner') {
